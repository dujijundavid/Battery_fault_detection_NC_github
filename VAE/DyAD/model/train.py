...
class Train_fivefold:
    ...
    def main(self):
        ...
        """
        print("Indexing dataset (lazy mode, no bulk loading)...")
        data_pre = dataset.Dataset(self.args.train_path, train=True, fold_num=self.fold_num)
        # —— 仅用抽样做归一化统计 —— #
        norm_sample_size = int(getattr(self.args, "norm_sample_size", 100000))
        norm_sample_size = min(norm_sample_size, len(data_pre))
        rng = np.random.default_rng(42)
        sample_idx = rng.choice(len(data_pre), size=norm_sample_size, replace=False)
        dfs_sample = []
        for i in tqdm(sample_idx, desc=f"Loading {norm_sample_size} samples for normalization", ncols=120):
            dfs_sample.append(data_pre[int(i)][0])
        self.normalizer = Normalizer(dfs=dfs_sample, variable_length=self.args.variable_length)
        # —— 传入“瘦身”的归一化函数，避免把整个 normalizer 复制进多进程 —— #
        norm_func = getattr(self.normalizer, "norm_func", None)
        # 尝试用 mean/std 构造轻量函数；若不可用则回退到原 norm_func
        try:
            mean = np.asarray(getattr(self.normalizer, "mean_"))
            std = np.asarray(getattr(self.normalizer, "std_"))
            if mean is not None and std is not None:
                def _light_norm(x, mean=mean, std=std):
                    return (x - mean) / (std + 1e-8)
                norm_func = _light_norm
        except Exception:
            pass
        train = PreprocessNormalizer(data_pre, normalizer_fn=norm_func)
        ...
        # === DataLoader（训练：更稳、更省内存）===
        num_workers_train = min(8, int(self.args.jobs)) # Windows/NAS 建议 ≤8~12，这里先 ≤8
        data_loader = DataLoader(
            dataset=train,
            batch_size=self.args.batch_size,
            shuffle=True,
            num_workers=num_workers_train,
            drop_last=True, # 等长 batch，减少末轮抖动
            pin_memory=torch.cuda.is_available(),
            collate_fn=collate if self.args.variable_length else None,
            prefetch_factor=2 if num_workers_train > 0 else None,
            persistent_workers=False # 关闭常驻，避免RAM常驻增长
        )
        # === 自动设置 KL 退火中心 x0（按总 step 的 70%） ===
        total_steps = len(data_loader) * self.args.epochs
        if getattr(self.args, "x0", 0) == 0:
            self.args.x0 = int(total_steps * 0.7)
        # === 学习率调度：真正退火到 cosine_factor × lr ===
        scheduler = CosineAnnealingLR(
            optimizer,
            T_max=self.args.epochs,
            eta_min=self.args.learning_rate * self.args.cosine_factor # 建议把配置里的 cosine_factor 设为 0.1
        )
        self.label_data = tasks.Label(column_name="mileage", training_set=train)
        time_start = time.time()
        try:
            p_bar = tqdm(total=len(data_loader) * self.args.epochs,
                         desc='training', ncols=160, mininterval=1, maxinterval=10, miniters=1)
            while self.current_epoch <= self.args.epochs:
                ...
                for batch in data_loader:
                    ...
                    loss = (self.args.nll_weight * nll_loss
                            ...
                            + kl_weight * kl_loss / batch_.shape[0])
                    # update parameters + 梯度裁剪
                    optimizer.zero_grad()
                    loss.backward()
                    clip_grad_norm_(model.parameters(), 1.0) # ☆ 防止偶发大梯度
                    optimizer.step()
                    # calculate loss
                    ...
                    loss_info = {
                        'mean_loss': total_loss / (1 + iteration),
                        'nll_loss': total_nll / (1 + iteration),
                        "label_loss": total_label / (1 + iteration),
                        "kl_loss": total_kl / (1 + iteration)
                    }
                    current_lr = optimizer.param_groups[0]['lr']
                    p_bar.set_postfix({**loss_info, "lr": f"{current_lr:.2e}", "kl_w": f"{kl_weight:.3f}"})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str,
                        default=os.path.join(
                            ...
                            r'C:\Users\YIFSHEN\Documents\VAE\DyAD\model_params_battery_brandall.json'
                        ))
    parser.add_argument('--fold_num', type=int, default=0) # 加入fold_num参数
    ...
    print("args", args)
    # 用Train_fivefold替换Train，传递fold_num
    Train_fivefold(args, fold_num=args.fold_num).main()