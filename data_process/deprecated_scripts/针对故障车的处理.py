...
def copy_specified_folders(source_root, target_root, folder_names):
    """
    复制指定的文件夹及其内容到目标目录
    
    Args:
        source_root (str): 源目录根路径
        target_root (str): 目标目录根路径
        folder_names (list): 要复制的文件夹名称列表
    """
    # 确保目标目录存在，如果不存在则创建
    ...
    for folder_name in folder_names:
        ...
        try:
            ...
            if os.path.exists(target_folder):
                print(f"警告: 目标文件夹已存在，将跳过 - {target_folder}")
                continue
            
            # 复制文件夹及其所有内容（递归复制）
            shutil.copytree(source_folder, target_folder)
            print(f"成功复制: {source_folder} -> {target_folder}")
        
        except PermissionError:
            print(f"错误: 没有权限访问 - {source_folder}")
        except Exception as e:
            print(f"错误: 复制 {source_folder} 时发生异常 - {str(e)}")

if __name__ == "__main__":
    # 配置路径和要复制的文件夹名称
    source_root = "/Volumes/conf/dl/vol_prediction-rdeb-yifshen_common_blob/203_Validation_new/"
    target_root = "/Volumes/conf/dl/vol_prediction-rdeb-yifshen_common_blob/203_Validation_TR/"
    folders_to_copy = [
        "0_3997",
        "0_3581",
        "0_3927",
    ...
    # 执行复制操作
    print("开始复制文件夹...")
    copy_specified_folders(source_root, target_root, folders_to_copy)
    print("复制操作完成！")
...
def main():
    ...
    ap.add_argument("--raw_folder", type=str, required=False,
                    default=r"/Volumes/conf/dl/vol_prediction-rdeb-yifshen_common_blob/203_Validation_TR/",
                    help="根目录（下含 车号/*.parquet）")
    ap.add_argument("--out_folder", type=str, required=False,
                    default=r"/Volumes/conf/dl/vol_prediction-rdeb-yifshen_common_blob/203_Validation_TR_pkl//",
                    help="输出目录")
