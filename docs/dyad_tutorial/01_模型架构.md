...
graph TB
    ...
    subgraph 输出
        D4 --> O1[重构序列<br/>log_p: batch x seq x 4]
...
```mermaid
graph TD
    X[x1, x2, x3, x4, x5] --> F[前向 RNN<br/>h1, h2, h3...]
    X --> B[后向 RNN<br/>h1_prime, h2_prime, h3_prime...]

    F --> C[拼接:<br/>h_t = h_t_forward + h_t_backward]
    B --> C
...
```mermaid
graph LR
    M[潜在均值 mu<br/>latent_dim=8] --> N1[降维到<br/>hidden/2=64]
    N1 --> N2[ReLU 非线性]
