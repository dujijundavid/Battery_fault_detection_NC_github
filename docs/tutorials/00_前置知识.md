...
```mermaid
graph TD
    A[输入 x_t, h_t-1, c_t-1] --> B{遗忘门 f_t<br/>决定保留什么}
    A --> C{输入门 i_t<br/>决定添加什么}
    A --> D{输出门 o_t<br/>决定输出什么}

    B --> E[候选状态 c_tilde_t]
    C --> E
    D --> E
    E --> F[细胞状态 c_t = f_t · c_t-1 + i_t · c_tilde_t]
    F --> G[隐藏状态 h_t = o_t · tanh(c_t)]
...
```mermaid
graph TD
    X[x1, x2, x3, ..., x_t] --> Forward[前向 RNN<br/>h_t_forward]
    X --> Backward[后向 RNN<br/>h_t_backward]

    Forward --> Concat[拼接:<br/>h_t = h_t_forward + h_t_backward]
    Backward --> Concat
